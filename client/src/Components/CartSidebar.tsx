...
const CartSidebar = () => {
  ...
  return (
    ...
    <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
      ...
      <div className="flex items-center justify-between p-5 border-b border-app-green">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <ShoppingBag className="size-16 mb-4 text-app-border"/>
            <h4 className="text-lg font-medium mb-1">Your Cart is Empty</h4>
          </div>
        ) : (
          items.map((item) => (
            <div key={item.product._id} className="flex gpa-3 bg-app-cream/60 rounded-xl p-3">
      ...
      {items.length > 0 && (
        <div className="p-5 border-t border-app-border space-y-3">
          ...
          {deliveryFee > 0 && <p className="text-xs text-app-text-light text-center">Free deliveries on orders are maxed {currency}20!</p>}
          ...
          className="w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]">
            Checkout Here <ArrowRight className="size-4"/>
          </button>
