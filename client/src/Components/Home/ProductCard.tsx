    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow hover:shadow-md transition-all duration-300 group animate-fade-in cursor-pointer" onClick={() => navigate(`/products/${product._id
      ...
      <div className="relative aspect-square overflow-hidden">
         ...
         <div className="absolute top-3 left-3 flex flex-wrap gap-2">
           {product.discount > 0 && (
             <span className="px-2 py-o.5 text-[10px] font-semibold uppercase bg-app-orange-dark text-white rounded-full"> {product.discount} % OFF </span>
           )}
           
         </div>
      </div>
       {/* Product Details */}
       <div className="p-3.5 tex-zinc-700">
         <h3 className="text-sm leading-snug mb-2 line-clamp-2">{product.name}</h3>
