...
export const inngest = new Inngest({ id: "enzo-cart" });

// Low stock alert to {Admin} when product stock is low:
const lowStockAlert = inngest.createFunction(
  ...
  async ({ event, step }) => {
   ...
   await step.run("send low stock alert email", async() => {
    // Send email to admin
    const adminEmail = process.env.ADMIN_EMAILS ? process.env.ADMIN_EMAILS.split(",").map((e) => e.trim()) : [];
