...
const generateToken = (id: string) => {
    return jwt.sign({id}, process.env.JWT_SECRET as string, {expiresIn: "60d"})
}
...
export const register = async (req: Request, res: Response) => {
    ...
    if(existingUser) {
        return res.status(400).json({ message: "User already exists, check your email." })
    }
...
// Login
//POST / api / auth / login
export const logn = async (req: Request, res: Response) => {
